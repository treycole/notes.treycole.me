---
created: 2025-04-08T17:28
modified: 2025-04-10T02:10
tags:
  - Physics
  - Condensed-Matter
---
...
$$
 \mathbf{M} = \frac{1}{2cV_{cell}} \int_{cell} \mathbf{r} \times \mathbf{j}(\mathbf{r}) d^3r
$$
 where the integral in over the interior of the unit cell. However, this will depend on the choice of unit cell. Even perfect knowledge of the periodic bulk $\mathbf{j}$ is insufficient for determining the orbital magnetization (Hirst, 1997).
...
$$
\mathbf{m} = \frac{-e}{2c} \sum_{n\in occ} \langle \psi_n | \mathbf{r} \times \mathbf{v}  |\psi_n \rangle
$$
where $\mathbf{v}$ is the velocity operator. For a crystal, the position operator is ill defined in Bloch representation. Analogous to the modern theory of polarization, we can hope to express it in terms of a Berry connection. 
...
$$
M =  \frac{e}{\hbar c} \frac{1}{(2\pi)^2} \sum_n \int_{BZ} \text{Im} \langle \partial_x u_{n\mathbf{k}} | H_{\mathbf{k}} + E_{n\mathbf{k}} - 2E_F | \partial_y   u_{n\mathbf{k}} \rangle f_{n\mathbf{k}} d^2 k
$$
where $\partial_u = \partial/\partial k_{\mu}$. The term involving $E_F$ is proportional to the total Chern number for an insulator, and can be dropped for trivial insulators. 

In 3D, $\mathbf{M}_{orb}$ is a pseudovector, giving rise to a 2D surface sheet current $\mathbf{K}_{surf} = \mathbf{M}_{orb} \times \hat{n}$ for surface normal $\hat{n}$. We can simply average over wavevectors in spectator directions. For magnetization, this leads to the 3D formula
$$
M_{\alpha} = \frac{e}{2\hbar c} \frac{1}{(2\pi)^3} \sum_n \int_{BZ} \text{Im}\ 
 \epsilon_{\alpha \mu\nu} \langle \partial_\mu u_{n\mathbf{k}} | H_{\mathbf{k}} + E_{n\mathbf{k}} - 2E_F | \partial_{\nu} u_{n\mathbf{k}} \rangle d^3 k
$$
We can also write it as
$$
\mathbf{M} = \frac{e}{2\hbar c} \frac{1}{(2\pi)^3} \sum_n \int_{BZ} \text{Im}\ 
  \langle \nabla_{\mathbf{k}} u_{n\mathbf{k}} | \times  H_{\mathbf{k}} + E_{n\mathbf{k}} - 2E_F |\nabla_{\mathbf{k}} u_{n\mathbf{k}} \rangle d^3 k
$$
...
Polarization in 1D constrains the surface charge $Q_{surf}$ modulo a charge quantum $e$. If we want to know the $P_x$ component in 2D or 3D, connected to edge or surface charges, then we use the 1D Berry-phase theory in the $x$ direction and average over wavevectors in other "spectator" directions.

Orbital magnetization first arises in 2D, since there are no circulating currents in 1D. $M_{orb}$ is a pseudoscalar (invariant under inversion but odd under TR) that determines the edge current $I$. This time there is _no ambiguity modulo a quantum_. The edge current is $I=M_{orb}$ independent of edge conditions. 
